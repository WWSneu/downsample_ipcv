...
class VisionAttention(nn.Module):
    ...
    def forward(
        ...
        return {'attn_output':attn_output,'k_states':k,'attn_scores':attn_weights}
...
class Qwen2VLVisionBlock(nn.Module):
    def __init__(self, config, layer_idx,attn_implementation: str = "sdpa") -> None:
        ...
        # Calculation amount 4*D²·L + D·L²  D=1280
        self.attn = QWEN2_VL_VISION_ATTENTION_CLASSES[attn_implementation](
            config.embed_dim, num_heads=config.num_heads,
        )
        #print("mlp dim",config.embed_dim,mlp_hidden_dim) # mlp dim 1280 5120
        # Calculation amount 8LD² seq_len * 1280 * 5120 * 2  8/5*attention?
    ...
    def forward(self, hidden_states_pkg : dict, cu_seqlens, rotary_pos_emb,sparse_vit_saved=None) -> torch.Tensor:
        if sparse_vit_saved is not None:
            ...
            # 2) compute delta for each unique_idx 
            delta_unique = (new_kept[unique_idx] - orig_kept[unique_idx])              # [U, D]

            # 3) reshape inv_idx  to (R, topk), compute the mean of delta
...
class Qwen2VLAttention(nn.Module):
    ...
    def forward(
        ...
        #print("eager output attn",output_attentions)
...
class Qwen2VLDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2VLConfig, layer_idx: int, attn_implementation: str = None):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        **kwargs,
        ...
        #print("output_attenion",output_attentions )
        ...
        if output_attentions:
            outputs['attn_scores'] = self_attn_weights

        if use_cache:
            outputs['past_key_value'] = present_key_value

        outputs['query_states'] = query_states
        outputs['key_states'] = key_states
        outputs['value_states'] = value_states
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config) -> None:
        ...
        #print("_attn_implementation",config._attn_implementation) # flash_attention_2
        self.blocks = nn.ModuleList(
            [Qwen2VLVisionBlock(config,layer_idx ,config._attn_implementation) for layer_idx in range(config.depth)]
        )
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        # grid_thw[batch_size, 3(t,h,w)] or [num_images, 3]
        hidden_states = self.patch_embed(hidden_states) 
        rotary_pos_emb = self.rot_pos_emb(grid_thw) # [seq_len, rot_pos_dim?]
...
class IPCV_ViT(Qwen2VisionTransformerPretrainedModel):
    """
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, inputs_downsample: torch.Tensor = None) -> torch.Tensor:
        # hidden_states [grid_t * grid_h * grid_w, 
        #       channel * self.temporal_patch_size * self.patch_size * self.patch_size] 
        ...
        hidden_states_downsample = self.patch_embed(inputs_downsample["pixel_values"]) # [seq_len, embed_dim]
        rotary_pos_emb_downsample = self.rot_pos_emb(inputs_downsample['image_grid_thw']) # [seq_len, rot_pos_embed_dim]
        cu_seqlens_downsample = torch.repeat_interleave(inputs_downsample['image_grid_thw'][:, 1] * inputs_downsample['image_grid_thw'][:, 2], inputs_downsample['image_grid_thw'][:, 0]).cumsum(
            dim=0, dtype=torch.int32
        )
        cu_seqlens_downsample = F.pad(cu_seqlens_downsample, (1, 0), value=0) # [1+Total Frames] Record the beginning and end index information of different frame images
        cu_seqlens_downsample = cu_seqlens_downsample.to(hidden_states.device)

        cu_seqlens = torch.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2], grid_thw[:, 0]).cumsum(
            dim=0, dtype=torch.int32
        )
        cu_seqlens = F.pad(cu_seqlens, (1, 0), value=0) # [1+Total Frames] Record the beginning and end index information of different frame images
        cu_seqlens = cu_seqlens.to(hidden_states.device)
        hidden_states_downsample = {'hidden_states': hidden_states_downsample, 'k_states': None, 'attn_scores': None}
        for blk in self.blocks:
            hidden_states_downsample = blk(hidden_states_downsample, cu_seqlens=cu_seqlens_downsample, rotary_pos_emb=rotary_pos_emb_downsample)
        ...
        hidden_states_pkg = {'hidden_states':hidden_states, # [seq_len, embed_dim]
                            ...
                            'attn_scores':None}                 # [nheads,seqlen,seqlen] 
        frame_counts = torch.zeros(1, device=device)
        hidden_states_prev = None

        # for blk in self.blocks:
        #     # Wrap hidden_states_downsample in a dict as expected by Qwen2VLVisionBlock
        #     pkg = {'hidden_states': hidden_states_downsample, 'k_states': None, 'attn_scores': None}
        #     res = blk(pkg, cu_seqlens=cu_seqlens, rotary_pos_emb=rotary_pos_emb)
        #     hidden_states_downsample = res['hidden_states']
        # for blk in self.blocks:
        #     hidden_states = blk(
        #         hidden_states_downsample,
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if Sparse_config is not None and Sparse_config['vit_Sparse']:
                ...
                if K-1>0 and blk.layer_idx ==K-1 and hidden_states_pkg['hidden_states'].shape[0]>1:
                    hidden_states_prev = hidden_states_pkg['hidden_states'] # K-1 layer states
                ...
                if blk.layer_idx == K and hidden_states_pkg['hidden_states'].shape[0]>1:
                    ...
                    hidden_states_cur = hidden_states_pkg['hidden_states'] # K layer states
                    ...
                    # Save the original sequence length, original hidden_states, removed index and their corresponding vectors 
                    ...
                    # ========== Calculate the 10 nearest neighbors of each removed token in kept_states ==========
                    with torch.no_grad():
                        dists = torch.cdist(
                        ...
                        # topk minimum distance index of kept_states [R, 10]
                        Top_K = Sparse_config['Top_K']
                        _, rem_to_kept_idx = dists.topk(min(Top_K,orig_kept_states.shape[0]), largest=False, dim=1)
                    ...
                    # compute cu_seqlens after pruning
        ...
        # ------------------------END------------------------------------------
            ...
            if hasattr(self, "_sparse_vit_saved") and blk.layer_idx < Sparse_config['vit_pruned_layer'] + Sparse_config['AS_layer']: # 7
                # layer pruned with AS
            ...
            if blk.layer_idx == self.config.depth-1 and hasattr(self, "_sparse_vit_saved"):
...
class Qwen2VLModel_Sparse(Qwen2VLModel):
    def __init__(self, config: Qwen2VLConfig):
        self.last_attention = None
        super().__init__(config)
        self.config = config
    ...
    def forward(
        ...
        **kwargs,
        ...
        #print("posi",position_ids.shape,position_ids,cache_position)
        if position_ids is None: 
            # the hard coded `3` is for temporal, height and width.
            position_ids = cache_position.view(1, 1, -1).expand(3, inputs_embeds.shape[0], -1)
        ...
        #print("causal_mask",causal_mask) # None
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                Sparse_config = self.config.Sparse_config
                if Sparse_config is not None and Sparse_config['Sparse']:
                    K = Sparse_config['pruned_layer']  # pruned layer
                    image_token_start_index = Sparse_config['image_token_start_index']
                    image_token_length = Sparse_config['image_token_length']
                    ...
                    if decoder_layer.self_attn.layer_idx == K and seq_length > 1:
                        ...
                        retained_image_tokens_index = self.get_retained_image_token_DART(
        ...
        next_cache = next_decoder_cache if use_cache else None
        #print("next_cache",next_cache,use_cache) # next_cache DynamicCache() True
    ...
    def get_retained_image_token_DART(self, config: Qwen2VLConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
        # prune with DART
        Sparse_config = config.Sparse_config
        ...
        pivot_image_token = Sparse_config['pivot_image_token']
        pivot_text_token = Sparse_config['pivot_text_token']

        reduction_ratio = Sparse_config['reduction_ratio']
        ...
        k_states_image_token = any_states[image_token_start_index:image_token_start_index + image_token_length, :]
        ...
        indices_set = set(image_indices + query_indices)#Select pivots for image and text

        valid_indices = set(range(image_token_start_index, image_token_start_index + image_token_length)) - set(image_indices)  # only prune in the image
        ...
        for item in list(indices_set):
            ...
            valid_indices.difference_update(top_k_real_indices) #equivalent to   valid_indices = valid_indices - set(top_k_real_indices)
            valid_indices_list = list(valid_indices)  
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # )
        #print("conf",config,config.vision_config)
        self.visual = IPCV_ViT._from_config(
            config.vision_config, attn_implementation=config._attn_implementation
        )
        # self.visual_downsample = Qwen2VisionTransformerPretrainedModel._from_config(
        #     config.vision_config, attn_implementation=config._attn_implementation
        # )        
        #self.model = Qwen2VLModel(config)
        self.model = Qwen2VLModel_Sparse(config)
        self.vocab_size = config.vocab_size
