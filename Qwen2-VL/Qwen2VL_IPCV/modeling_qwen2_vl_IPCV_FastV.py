...
class IPCV_ViT(Qwen2VisionTransformerPretrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if Sparse_config is not None and Sparse_config['vit_Sparse']:
                ...
                if blk.layer_idx == K and hidden_states_pkg['hidden_states'].shape[0]>1:
                    ...
                    # update frame count
                    num_frames = len(cu_seqlens) - 1  # frame count
                    ...
                    #print("frame",cu_seqlens,frame_counts)tensor([   0, 5220], device='cuda:0', dtype=torch.int32) tensor([4176], device='cuda:0')
        ...
        return self.merger(hidden_states_pkg['hidden_states']) 
...
class Qwen2VLModel_Sparse(Qwen2VLModel):
    ...
    def forward(
        ...
        # Trigger only once
        if self.config.Sparse_config is not None and self.config.Sparse_config['Sparse'] \
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if Sparse_config is not None and Sparse_config['Sparse']:
                    ...
                    if decoder_layer.self_attn.layer_idx == K and seq_length > 1:
                        ...
                        retained_image_tokens_index = self.get_retained_image_token_FastV(
    ...
    def get_retained_image_token_FastV(self, config: Qwen2VLConfig,
    ...
    def update_layer(self, device=None, dtype=None):
        k = self.config.Sparse_config['pruned_layer'] - 1
        if k<0 : return 

        old_block = self.layers[k]
        # Create a new block and use 'eager' attention 
