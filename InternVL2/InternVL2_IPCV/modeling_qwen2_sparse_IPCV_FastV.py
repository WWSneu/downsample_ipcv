...
class Qwen2Model_Sparse(Qwen2Model):
    ...
    def get_retained_image_token_FastV(self, config, last_layer_state: torch.Tensor, any_states: torch.Tensor, attn_scores: torch.Tensor) -> torch.Tensor:
        # prune with FastV
        ...
        # attn_scores = attn_scores.sum(dim=-2) # Sum along the query dimension
        attn_scores = attn_scores.mean(dim=0) # Average different attention heads

        last_tok_attn = attn_scores[-1]
        
        # Take out the image token
        img_scores = last_tok_attn[image_token_start_index:image_token_start_index + image_token_length]

        # Select the top-k image token (relative to the index of the image token segment)
